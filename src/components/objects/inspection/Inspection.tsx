...
import { networkSynchronizer } from "../../primitives/NetworkSync"
...
function InspectionSection(props: InspectionSectionProps): JSX.Element {
    ...
    if (props.hideComplete && props.childRequirements.every((req) => req.met === true)) {
        return <></>
    }
...
export function Inspection(props: InspectionProps): JSX.Element {
    ...
    const valuesFromNetwork: InspectionStatus = props.sections.reduce((acc, section) => {   
        acc[section.uuid] = section.childRequirements.reduce((acc, req) => {
