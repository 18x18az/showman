...
export function SaveBar(props: SaveBarProps): JSX.Element {
    let lockIcon = props.locked === true ? <LockClosedIcon className={`${SIZE} text-slate-12`}/> : <LockOpen2Icon className={`${SIZE} text-slate-12`}/>
    const canSave = props.locked === true

    const eyeIcon = props.hidden === true ? <EyeClosedIcon className={`${SIZE} text-slate-12`}/> : <EyeOpenIcon className={`${SIZE} text-slate-12`}/>

    const saveTextColor = canSave ? "text-slate-12" : "text-slate-9"

    return <div className="flex justify-center gap-x-16 my-4">
