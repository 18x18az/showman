...
function FieldControl (props: FieldControlProps): JSX.Element {
  ...
  const outlineStrength = isCurrent ? 'border-zinc-500' : 'border-zinc-800'
  ...
  if (isCurrent) {
    if (status.state === FieldState.ON_DECK) {
      ...
      actionButton = (
        <><Button
          className='w-24' onClick={() => {
            if (status.match === null) return
            start(status.match.id)
          }}
          >Reset
        </Button><Button className='w-24' onClick={resume}>Resume</Button>
        </>
      )
    }
  }
