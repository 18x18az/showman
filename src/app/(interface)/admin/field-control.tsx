...
import { Alliance, Field, FieldStatus, Match } from "../interfaces"
...
function ReplayButton(props: {match: Match}): JSX.Element {
    return <DropdownMenuItem>Replay</DropdownMenuItem>
}

function QueueButton(props: {match: Match, target: Field}): JSX.Element {
    ...
    return <DropdownMenuItem onClick={() => {void handle()}} className={`${color}`}>Queue on {props.target.name}</DropdownMenuItem>
}
...
export function FieldDisplay(props: MatchDisplayProps): JSX.Element {
    if(props.match === null) {
    ...
    const isOnCorrectField = props.match.fieldId === undefined || props.match.fieldId === props.fieldId
    ...
    props.openFields.forEach((field) => {
        if(field.id === props.fieldId) return
        options.push(<QueueButton match={props.match as Match} target={field} key={field.id}/>)
    })
...
export function FieldControl(): JSX.Element {
    ...
    const columns = statuses.map((status) => {
        return <div className="flex flex-col gap-8 w-96 mb-8" key={status.field.id}>
            ...
            <FieldDisplay fieldId={status.field.id} match={status.match} openFields={openFields}/>
            <QueueDisplay fieldId={status.field.id} match={status.onDeck} openFields={openFields}/>
        </div>
    })
