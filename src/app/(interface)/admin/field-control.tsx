...
function UnqueuedInfo (props: UnqueuedInfoProps): JSX.Element {
  ...
  return (
    <div className='rounded-md flex flex-col w-32 h-32 border border-zinc-800 justify-between'>
...
export function FieldDisplay (props: FieldDisplayProps): JSX.Element {
  if (props.match === null) {
    return (
      <div className='border border-zinc-900 rounded-md flex-1 mb-4 flex flex-col justify-evenly text-4xl h-full text-center text-zinc-800'>
        Empty
      </div>
    )
  }
  ...
  return (
    <div className={`border ${borderColor} rounded-md flex-1 mb-4 flex flex-col`}>
      <div className='flex justify-end'>
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
...
export function QueueDisplay (props: MatchDisplayProps): JSX.Element {
  ...
  return (
    <div className={`border ${borderColor} rounded-md flex-1 flex flex-col`}>
      <div className='flex justify-end'>
...
export function FieldControl (props: FieldControlProps): JSX.Element {
  ...
  const columns = statuses.map((status) => {
    return (
      <div className='flex flex-col gap-8 w-96 mb-8' key={status.field.id}>
  ...
  if (unqueued.length > 0) {
    ...
    if (allFieldsEmpty) {
      unqueuedInfo = (
        <div className='rounded-md flex-1 flex flex-col justify-center items-center h-full'>
          <Button onClick={() => { void EmptyPost('matches/proceed') }} variant='secondary' className='text-4xl p-6 py-8'>Queue Next Block</Button>
        </div>
      )
    } else {
      unqueuedInfo = (
        <div className='rounded-md flex-1 flex flex-col justify-center'>
